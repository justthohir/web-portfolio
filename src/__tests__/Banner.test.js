...
test("it shows the title in the banner", () => {
  setup();
  // We expect that the title 'Hi, Iam Thohir' is in the banner component
  expect(screen.getByText(/Hi, Iam Thohir/i)).toBeInTheDocument();
});
...
test("can download cv when clicked on download cv button", async () => {
  ...
  // expect(downloadLink.download).toEqual('M-Thohirin-Resume.pdf');
  // expect(downloadLink.href).toEqual('/files/M-Thohirin-Resume.pdf');
